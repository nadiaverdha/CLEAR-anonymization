...
Generated on: 2026-05-13T13:01:09.225162
...
```
 python benchmark.py --config reports/findok/Qwen_Qwen3.5-35B-A3B/organisation/2026-05-13_v5/config.yaml 
```
| Parameter | Value |
|---|---|
| Pool size | None |
...
| `company_generic_suffix` | 0.0% | 0.0% | 0.0% | 2 | 0 | 2 |
...
**Format:** `regex`  
**Description:**
Matches the specific web-based tax authority reference 'www.FA Salzburg-Stadt' found in training data.

**Content:**
```
\bwww\.FA\s+Salzburg\-Stadt\b
```
...
**Format:** `regex`  
**Description:**
Matches generic company names ending in common business suffixes, but requires a preceding proper noun (capitalized word) and excludes known specific patterns.

**Content:**
```
\b([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+)*)\s+(?:GmbH|AG|KG|Systeme|Maschinenbau|Recycling|Planung|Software|Sicherheit|Logistik|Handel|Dienstleistungen|Bau|Elektro|Garten|Touristik|Verlag|Vertrieb|Holding|Institut|Gruppe|Werke|Produkte|Technologien|Automotive|Cloud|KI|Medien|Immobilien|Versicherung|Pflege|Analyse|Gastronomie|Lebensmittel|Druck|Textil|Luftfahrt|Energie|Werkal|Werk|Betriebe|E-Commerce|ECommerce)\b(?!(?:\s+&\s*Co\s+KG|\s+GmbH\s+&\s*Co))
```
...
**Format:** `regex`  
**Description:**
Matches the specific entity 'Waldtra-Sicherheit' found in multiple failure cases.
...
**Format:** `regex`  
**Description:**
Matches 'Donau Furtkraftwald AG' and the URL variant '://www.Donau Furtkraftwald AG' found in training data.

**Content:**
```
(?:Donau\s+Furtkraftwald\s+AG|://www\.Donau\s+Furtkraftwald\s+AG)
```
